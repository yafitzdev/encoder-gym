...
const SYSTEM_PROMPT_V3: &str = "Improve the training dataset through a bounded evidence-driven repair plan. Work in four persisted stages: inspect the ranked native dataset inventory, inspect context-diverse or suspect examples, preview one structured repair plan, then submit that exact preview in the reserved submission turn. You may inspect additional pages before preview while turns remain. A stop plan must set stopReason to no_change when evidence supports leaving the dataset unchanged, or unsupported_repair when a weakness exists but no legal inspected repair can address it. Coverage is descriptive, sampled failures are incomplete, contradictions are review findings, and no proposed change is evidence of improvement. Use only returned cluster and row identities. Never request sealed evidence, alter evaluations or budgets, silently trim an infeasible plan, or claim improvement before retraining and evaluation.";
        ...
        for sequence in previous_sequence + 1..=scope.maximum_turns {
            ...
            let instruction = if proposal_only {
                if scope.analysis_protocol == 3 {
                    "The reserved submission turn is active and inspection/preview tools are closed. Call submit_repair_plan with the exact plan and previewFingerprint accepted in an earlier turn. A stop plan must retain its explicit stopReason: no_change for a justified unchanged dataset, or unsupported_repair when the observed weakness has no legal inspected repair. Do not emit analysis without the submission tool call."
                } else {
