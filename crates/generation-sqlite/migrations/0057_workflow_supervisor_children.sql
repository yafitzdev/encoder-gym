DROP TRIGGER workflow_child_executions_insert_authority;
DROP TRIGGER workflow_child_executions_immutable_update;
DROP TRIGGER workflow_child_executions_immutable_delete;

ALTER TABLE workflow_child_executions RENAME TO workflow_child_executions_legacy;
...
CREATE TABLE workflow_child_executions (
    ...
    child_kind TEXT NOT NULL CHECK (child_kind IN (
        'generation_job', 'generation_supervisor_run', 'quality_audit_run',
    ...
    UNIQUE (workflow_stage_attempt_id, child_kind, child_execution_id)
);
...
SELECT
    id, workflow_run_id, workflow_stage_attempt_id, stage, ordinal, child_kind,
    logical_key, child_execution_id, artifact_json, fingerprint, created_at
FROM workflow_child_executions_legacy;

DROP TABLE workflow_child_executions_legacy;
...
BEGIN
    ...
    SELECT CASE WHEN EXISTS (
        SELECT 1
        FROM workflow_child_executions
        WHERE child_kind = NEW.child_kind
          AND child_execution_id = NEW.child_execution_id
          AND (
              workflow_run_id != NEW.workflow_run_id
              OR stage != NEW.stage
              OR logical_key != NEW.logical_key
          )
    ) THEN RAISE(ABORT, 'workflow child execution cannot cross workflow authority') END;

    SELECT CASE WHEN NOT (
        (NEW.stage IN ('generation', 'dataset_diff_generation') AND NEW.child_kind IN ('generation_job', 'generation_supervisor_run'))
