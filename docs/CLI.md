...
Read `features/optimization/encoder-optimize.md` for the strict manifest fields
and the difference between starting a new run and adopting an existing one.
...
task/schema inference remains the fallback. See `features/datasets/semantic-catalog.md`.
...
`features/research/authenticity-research.md`.
...
or Pi call. See `features/datasets/dataset-architect-spec.md`.
...
The process-level test scenario uses the actual Pi JSONL process with a
deterministic local corpus. The agent sees no benchmark rows, predictions,
member identities, paths, or sealed diagnostics. A
...
`features/evaluation/benchmark-architect-spec.md`.
...
`features/datasets/dataset-quality-spec.md`.
...
`features/generation/generation-quality-supervisor.md`.

For the governed end-to-end workflow, select supervision in the project
preparation document. Its `[workflow.generation_supervision]` outcome controls and separate
...
workflow start <DEFINITION_ID>` command as a separate authorization. See
`features/datasets/project-preparation.md`.
...
Totals are absolute accepted-row targets. See `features/datasets/allocation.md` for policy,
...
- `synth workflow define --definition <WORKFLOW.toml>` to derive or reuse the global benchmark-bundle
...
See `features/evaluation/error-analysis.md` for metric semantics and complete examples.
...
`features/optimization/optimization.md` for semantics and the complete operator
workflow.

See `features/training/transformer-training.md` for the complete local BERT
workflow and non-secret settings.
