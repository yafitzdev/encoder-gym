...
cannot silently change a run. Generate a validated starting file with `config
init`, then inspect the resolved form before execution.

```text
cargo run -p synthetic-data-cli -- config init encoder-gym.toml
cargo run -p synthetic-data-cli -- config validate encoder-gym.toml
cargo run -p synthetic-data-cli -- --output json config show encoder-gym.toml
```
...
by `synth encoder register`. See
`docs/features/training/transformer-training.md`. Unknown fields are rejected at every level,
...
Validate and inspect a hybrid project configuration before generation:

```text
synth config validate <PROJECT_CONFIG.toml>
synth config construction-preview <PROJECT_CONFIG.toml> \
...
artifacts rather than mutable project defaults. The normalized protocol and fingerprint are
...
Create a workflow document with the persisted identities and fingerprints
returned after creating the project and benchmark suites.
