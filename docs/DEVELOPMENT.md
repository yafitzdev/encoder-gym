...
Reproducible project settings live in strict TOML; generate one with
`synth config init encoder-gym.toml` and read `docs/CONFIG.md`. Local process
settings begin with `.env.example`. `.env` and all `.env.*` files except the
example are ignored. Backend credentials come from the environment or the
server's process-local memory and are never written to TOML, SQLite,
fingerprints, CLI output, or logs.
...
temporary databases and do not reuse a developer database from another run.
...
upgrade test or with `synth doctor`.
...
bundle smoke command is documented in `docs/features/training/transformer-training.md` and must
