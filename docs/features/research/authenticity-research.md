...
```text
cargo run -p synthetic-data-cli -- research brief-validate <BRIEF.json>
```

Run the genuine Pi loop with its deterministic faux model and local corpus:

```text
cargo run -p synthetic-data-cli -- research start <BRIEF.json> --script <TURNS.json> --corpus <CORPUS.json>
```
