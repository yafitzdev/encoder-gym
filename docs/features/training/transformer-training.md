...
Set `training.backend = "bert-cpu"` and provide the registered `base_model_id`,
then run:

```text
synth config validate <PROJECT_CONFIG.toml>
synth training run <SNAPSHOT_ID> --config <PROJECT_CONFIG.toml>
```
...
synth doctor --config <PROJECT_CONFIG.toml>
```
