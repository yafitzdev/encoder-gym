...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    await check('saved cases load only on explicit request',`window.caseReads===0 && document.querySelector('.development-cases').textContent.includes('on demand')`);
    const prediction=rank=>({fingerprint:'sha256:'+'a'.repeat(64),question:'Find primary evidence for the current research question.',taskKind:'route',expectedCapabilities:['search'],predictedCapabilities:['write'],expectedRank:rank});
    const caseSource=id=>({reportId:id,reportFingerprint:'sha256:'+'b'.repeat(64),diagnosticsFingerprint:'sha256:'+'c'.repeat(64),support:200,sampleSize:45});
    const cases={projectId:f.projectId,runId:'agent-root',iterationId:'iteration-1',iteration:1,baselineModelId:'original-model',candidateModelId:'model-1',comparisons:[{suite:'development',suiteFingerprint:'sha256:'+'d'.repeat(64),sampleLimit:50,baseline:caseSource('original-report'),candidate:caseSource('candidate-report'),cases:Array.from({length:45},(_,index)=>({sourceRowId:'case-'+index,baseline:prediction(3),candidate:prediction(index%2===0?2:4),change:index%2===0?'rank_improved':'rank_regressed'}))}]};
    await evaluate(`window.caseFixture=${JSON.stringify(cases)};[...document.querySelectorAll('.development-cases button')].find(node=>node.textContent==='Compare saved cases').click()`);
    await check('saved cases keep both rank directions and exact model/report identities',`window.caseReads===1 && document.querySelectorAll('.saved-case').length===20 && document.querySelector('.development-cases').textContent.includes('23 rank improved · 22 rank regressed') && document.querySelector('.development-cases').textContent.includes('Original baseline model original-model') && document.querySelector('.development-cases').textContent.includes('candidate-report')`);
    for(const width of [1200,390]) {window.setContentSize(width,1000);await evaluate(`document.querySelector('.development-cases').scrollIntoView({block:'start'})`);await capture('saved-cases-'+width);await check('saved cases fit '+width,`document.documentElement.scrollWidth<=innerWidth`)}
    await evaluate(`[...document.querySelectorAll('.development-cases button')].find(node=>node.textContent==='Next cases').click();qa.render()`);
    await check('case pagination survives redraw without re-reading',`document.querySelector('.development-cases').textContent.includes('21–40 of 45') && window.caseReads===1`);
    await evaluate(`(()=>{const select=document.querySelector('.development-cases select[id$="-filter"]');select.value='rank_regressed';select.dispatchEvent(new Event('change',{bubbles:true}))})()`);
    await check('case filter retains regressions, not only improvements',`document.querySelectorAll('.saved-case h4.danger').length===20 && !document.querySelector('.saved-case h4.success')`);
    await selectIteration(2);await check('case evidence cannot leak to another iteration',`document.querySelector('.development-cases').textContent.includes('on demand') && !document.querySelector('.saved-case')`);
    await selectIteration(1);await check('returning to cases preserves the exact cached comparison',`document.querySelectorAll('.saved-case h4.danger').length===20 && window.caseReads===1`);
    await evaluate(`window.caseFixtureError=true;[...document.querySelectorAll('.development-cases button')].find(node=>node.textContent==='Reload saved cases').click()`);
    await check('read errors replace stale predictions with an explicit retry',`document.querySelector('.development-cases').textContent.includes('could not be read or verified') && !document.querySelector('.saved-case')`);
    await evaluate(`window.caseFixtureError=false;window.caseFixture.comparisons[0].baseline.sampleSize=null;window.caseFixture.comparisons[0].baseline.diagnosticsFingerprint=null;window.caseFixture.comparisons[0].cases=window.caseFixture.comparisons[0].cases.map(pair=>({...pair,baseline:null,change:'not_comparable'}));[...document.querySelectorAll('.development-cases button')].find(node=>node.textContent==='Retry saved cases').click()`);
    await evaluate(`(()=>{const select=document.querySelector('.development-cases select[id$="-filter"]');select.value='all';select.dispatchEvent(new Event('change',{bubbles:true}))})()`);
    await check('missing samples remain unknown and never become fabricated fixes',`document.querySelector('.development-cases').textContent.includes('Diagnostics unavailable; prediction unknown') && !document.querySelector('.saved-case h4.success')`);
    window.setContentSize(1440,1000);
