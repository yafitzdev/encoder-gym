...
export function parseInputOptimizationRun(value: unknown, expectedProjectId: string): InputOptimizationRun {
  ...
  bound(identity.launch); bound(identity.setup); instant(identity.createdAt); fingerprint(identity.fingerprint); fingerprint(item.headFingerprint);
  ...
  const run: InputOptimizationRun = {
    id, projectId, state: item.state as InputOptimizationState,
