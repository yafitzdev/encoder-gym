...
export class ManagedOptimizationSetup {
  ...
  async preview(projectId: string, value: unknown, progress?: (value: NativeProgress) => void, signal?: AbortSignal): Promise<OptimizationSetupPreview> {
    ...
    const workspace = await this.ports.open(projectId);
    const preview = await this.ports.command<OptimizationSetupPreview>(["optimization-setup", workspace.folder, "preview", "--model", modelId, "--dataset-version", datasetId, "--benchmark-version", benchmarkId], progress, signal);
  ...
  async save(projectId: string, value: unknown, progress?: (value: NativeProgress) => void, signal?: AbortSignal): Promise<OptimizationSetupSaved> {
    ...
    return this.ports.exclusive(projectId, async () => {
      ...
      try {
        ...
        const saved = await this.ports.command<OptimizationSetupSaved>(["optimization-setup", workspace.folder, "save", "--file", file], progress, signal);
