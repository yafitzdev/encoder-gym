...
export async function runManagedSmokeChecks(window: BrowserWindow, output: string, harness: Harness): Promise<void> {
  ...
  try {
    ...
    await check("Data distinguishes source custody from the frozen training snapshot", "document.querySelector('.page-heading h1').textContent === 'Data' && document.querySelector('.scientific-data-card').textContent.includes('24') && document.querySelector('.scientific-data-card').textContent.includes('approved repair rows') && document.querySelector('.scientific-data-card').textContent.includes('Prepared definition; no run reserved') && document.querySelector('.preparation-note').textContent.includes('not training-ready')");
