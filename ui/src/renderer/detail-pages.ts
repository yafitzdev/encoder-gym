...
import { bytesLabel, candidateDescription, candidateName, candidateRows, dateLabel, developmentStatus, durationLabel, metricInfo, resultSummary, runLabel, runName, score, delta, setupName, suiteName, timeLabel, type CandidateRow } from "./catalog.js";
...
export function renderResults(row: CandidateRow, state: DetailState, actions: Actions): HTMLElement {
  ...
  return h("div", {},
    h("div", { class: "result-banner" }, h("div", {}, status(s.label, s.tone), h("h2", {}, failed ? `${failed} of ${allChecks.length} development checks failed` : s.key === "passed" ? "All development requirements met" : "Evidence is incomplete"), h("p", {}, s.key === "passed" ? resultSummary(row) + "." : "The recorded requirements apply to every development suite independently.")), tag(`${c.development.length} / ${row.run.baselines.length} suites`)),
...
function runRecord(run: RunRecord, workspace: WorkspaceSnapshot, actions: Actions): HTMLElement {
  ...
  const command = run.optimizationId ? `synth encoder optimize report ${run.optimizationId} --workspace ${shellQuote(workspace.folder)}` : `synth encoder-experiment inspect ${run.id} --workspace ${shellQuote(workspace.folder)}`;
  ...
  return h("div", {}, h("div", { class: "detail-columns" },
    h("section", {}, sectionHeader("Recorded budget"), facts([["Candidate attempts", `${run.candidates.filter(c => c.model || c.failure).length} / ${run.budget.candidates}`], ["Completed build time", `${durationLabel(usedTraining)} / ${durationLabel(run.budget.trainingSeconds)}`], ["Development reports", `${run.candidates.reduce((n, c) => n + c.development.length, 0)} / ${run.budget.development}`], ["Final acceptance reports", `${["passed", "failed"].includes(run.acceptance.state) ? 1 : 0} / ${run.budget.sealed}`]])),
  ...
  ), sectionHeader("Inspect with the CLI"), h("p", { class: "section-note" }, "This command reads the full record. Training, approval, and model promotion are managed through the CLI."), copyField(command, actions.copy));
}
export function renderRun(workspace: WorkspaceSnapshot, id: string, tab: string, actions: Actions): HTMLElement {
  ...
  return h("div", { class: "page-content" }, button("All runs", () => actions.navigate({ page: "runs" }), "back-link", "back"),
    ...
    h("div", { id: "detail-panel", role: "tabpanel", "aria-labelledby": `tab-${tab}` },
      tab === "activity" ? h("ol", { class: "activity-list" }, ...run.activity.map(e => h("li", {}, h("span", { class: "activity-sequence" }, e.sequence), h("div", {}, h("strong", {}, eventLabel(e.kind)), e.candidateId ? h("span", {}, candidateName(run.candidates.find(c => c.id === e.candidateId)!)) : null), h("time", {}, timeLabel(e.at))))) : tab === "record" ? runRecord(run, workspace, actions) : h("div", {},
        h("div", { class: "run-summary" }, h("div", {}, h("div", { class: "eyebrow" }, "Recorded outcome"), h("h2", {}, run.decision === "retain_baseline" ? "The baseline was kept" : run.decision === "promote_candidate" ? "A candidate was promoted" : "No final decision recorded"),
          h("p", {}, failures.length ? "Execution errors were recorded. Open the candidate history for recovered attempts." : usedSealed ? "Development selected a candidate for final acceptance. The final assessment determined this outcome." : "No candidate was selected for final acceptance. The sealed evaluation was not run.")),
  ...
  return h("div", { class: "page-content" }, button("All models", () => actions.navigate({ page: "models" }), "back-link", "back"),
    ...
    ...reference.baselines.map(base => h("section", { class: "compare-matrix" }, sectionHeader(suiteName(base.suite)),
      h("div", { class: "table-scroll", tabindex: "0", "aria-label": "Side-by-side model comparison" }, h("table", { class: "comparison-matrix" },
        ...
        h("tbody", {},
          ...Object.keys(base.metrics).sort((a, b) => (["mrr", "recall_at_1", "recall_at_2", "recall_at_3", "agent_success_rate"].indexOf(a) + 1 || 99) - (["mrr", "recall_at_1", "recall_at_2", "recall_at_3", "agent_success_rate"].indexOf(b) + 1 || 99)).map(k => h("tr", {},
