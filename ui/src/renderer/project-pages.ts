...
export function renderProjectState(project: ProjectEntry, opened: OpenedProject | undefined, actions: Actions, projects: ProjectActions, page = "models"): HTMLElement {
  ...
  if (content.state === "error") return h("div", { class: "page-content" }, pageHeader(project.name, "This project couldn't be opened."),
    ...
    h("p", { class: "table-footnote" }, "Opening a project does not change its files."));
...
export function renderProjectSettings(project: ProjectEntry, opened: OpenedProject | undefined, actions: Actions, projects: ProjectActions): HTMLElement {
  ...
  return h("div", { class: "page-content" }, pageHeader("Project settings", managed ? "Locate this managed workspace or manage its library entry." : "Manage this read-only experiment connection."),
    ...
    h("section", { class: "project-info" }, sectionHeader("Evidence source", tag(!workspace ? opened?.content.state === "error" ? "Unavailable" : "No records yet" : workspace.source === "local" ? "Local journals" : "Recorded snapshot")),
      workspace ? h("div", {}, h("p", { class: "section-note" }, workspace.source === "local" ? "Read from this project's local databases. Reload to read newly persisted results." : "Historical example data, not a connection to the original workspace or its current state."), facts([["Read / captured", `${dateLabel(workspace.capturedAt)} at ${timeLabel(workspace.capturedAt)}`], ["Databases", h("div", { class: "stack" }, ...workspace.databases.map(file => h("code", {}, file)))]])) : opened?.content.state === "error" ? failureNotice(opened.content.message) : h("p", { class: "section-note" }, managed ? "Reading the workspace…" : "No baseline or experiment records have been found in this legacy folder."),
      button("Reload evidence", actions.refresh, "secondary", "refresh")),
    managed ? null : h("section", { class: "reading-note" }, h("h2", {}, "Read-only experiment evidence"), h("p", {}, "Renaming or reconnecting this project changes only the app's folder collection. Training, model registration, and final acceptance remain explicit CLI workflows."), h("p", {}, "The reader checks report bindings and journal continuity. It does not replace CLI Doctor's native artifact verification.")),
