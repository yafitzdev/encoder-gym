...
export function executeObservedCommand(executable: string, args: string[], environment: Readonly<Record<string, string>> | undefined, receive: (progress: NativeProgress) => void): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    let stdout = "", diagnostic = "", tooLarge = false;
    ...
    child.on("close", (code, signal) => {
      lines.finish();
      if (tooLarge) reject(new Error("The backend response exceeded its size limit."));
      else if (code !== 0) reject(new Error(diagnostic.trim() || `Run worker stopped (${signal ?? code}).`));
      else resolve(stdout);
    });
  });
}
